...
func (dm *DockerManager) runContainerInPod(pod *api.Pod, container *api.Container, netMode, ipcMode string) (kubeletTypes.DockerID, error) {
	...
	// Some containers are part of top-tier pods, and some containers are part of best-effort pods.
	// Processes in best-effort pods should be killed first if the system runs out of memory.
	// The main pod infrastructure container should be the last to get killed.
	var oomScoreAdj int
	if container.Name == PodInfraContainerName {
	...
	cgroupName, err := getFullContainerName(containerInfo.State.Pid)
	if err != nil {
		return "", err
	}
